...
async def test_shutdown_flushes_state(state_file, tmp_path):
    ...
    plugins_dir = tmp_path / "plugins_cache"
    plugins_dir.mkdir()
    # Use distinct ports so this test does not contend with the started_broker
    # fixture or with parallel worktree test runs.
    ui_port = UI_PORT + 20
    ...
    try:
        ...
        # Do NOT wait for the debounced background write — stop() must flush.
    finally:
        # Equivalent to what the SIGTERM handler does: enter the shutdown path.
        await b.stop()
        await asyncio.sleep(0.05)

    # Reload state from disk; the message must be present.
